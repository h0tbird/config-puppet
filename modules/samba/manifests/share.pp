...
define samba::share (
    ...
    if ! ( $writeable in [ 'yes', 'no' ] ) { 
        fail ( "${module_name}::share 'writeable' must be one of: 'yes' or 'no'" )
    }
  
    if ! ( $browseable in [ 'yes', 'no' ] ) { 
        fail ( "${module_name}::share 'browseable' must be one of: 'yes' or 'no'" )
    }
